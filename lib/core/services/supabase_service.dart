...
class SupabaseService {
  ...
  static Future<void> insertProducts(List<Map<String, dynamic>> rows) async {
    if (rows.isEmpty) return;
    await client.from('products').insert(rows);
  }

  static Future<List<Map<String, dynamic>>> fetchProductEnquiries() async {
    final response = await client
  ...
  static Future<void> createProductEnquiry({
    ...
    if (uid == null) throw Exception('Please login to send an enquiry.');
    await client.from('product_enquiries').insert({
      'user_id': uid,
  ...
  static Future<void> updateProductEnquiry(
    String id,
    Map<String, dynamic> data,
  ) async {
    await client.from('product_enquiries').update(data).eq('id', id);
  }

  static Future<String> uploadProductEnquiryImageBytes({
    required Uint8List bytes,
    required String fileName,
  }) async {
    const bucket = 'product-enquiries';
    final storagePath =
        'enquiries/${currentUser?.id ?? 'guest'}_${DateTime.now().millisecondsSinceEpoch}_$fileName';
    final ext =
  ...
  static Future<void> upsertBanner(Map<String, dynamic> data) async {
    ...
    if ((payload['id']?.toString().isEmpty ?? true)) {
      ...
      await client.from('banners').update(payload).eq('id', payload['id']);
    }
  }
  ...
  static Future<bool> _saveOrderSnapshot(Map<String, dynamic> order) async {
    ...
    orders.removeWhere(
      (row) => (row['order_ref']?.toString() ?? row['id']?.toString()) == key,
    );
  ...
  static Future<void> addProductCategory(String name) async {
    await client.from('product_categories').insert({'name': name.trim()});
  }

  static Future<void> updateProductCategory(String id, String name) async {
    await client
        .from('product_categories')
  ...
  static Future<List<Map<String, dynamic>>> fetchLowStockProducts({
    int threshold = 5,
  }) async {
    final response = await client
        .from('products')
  ...
  static Future<void> customerUpdateOrderStatus({
  ...
  static Future<List<Map<String, dynamic>>> fetchRecentOrders({
    ...
    try {
      final response = await client
          .from('orders')
          .select(
            'id, order_ref, user_id, status, subtotal, tax, total, created_at, whatsapp_number, delivery_address, payment_method, order_items(product_name, quantity, unit_price)',
          )
          .order('created_at', ascending: false)
          .limit(limit);
      return List<Map<String, dynamic>>.from(response);
      ...
      try {
        final response = await client
            .from('orders')
            .select(
              'id, user_id, status, total, created_at, whatsapp_number, delivery_address, payment_method, order_items(product_name, quantity, unit_price)',
            )
            .order('created_at', ascending: false)
            .limit(limit);
        return List<Map<String, dynamic>>.from(response);
        ...
        try {
          final response = await client
              .from('orders')
              .select(
                'id, order_ref, user_id, status, total, created_at, whatsapp_number, delivery_address',
              )
              .order('created_at', ascending: false)
              .limit(limit);
          return List<Map<String, dynamic>>.from(
            response,
          ).map((row) => {...row, 'order_items': const []}).toList();
        } catch (_) {
          return [];
        }
      }
  ...
  static Future<List<Map<String, dynamic>>> fetchAllOrders() async {
    try {
      final response = await client
          .from('orders')
          .select(
            'id, order_ref, user_id, status, subtotal, tax, total, created_at, updated_at, whatsapp_number, delivery_address, payment_method, courier_name, tracking_number, estimated_delivery, status_note, admin_note, order_items(product_name, quantity, unit_price)',
          )
          .order('created_at', ascending: false);
      return List<Map<String, dynamic>>.from(response);
    } catch (_) {
      try {
        ...
        try {
          final response = await client
              .from('orders')
              .select(
                'id, user_id, status, total, created_at, whatsapp_number, delivery_address, payment_method, order_items(product_name, quantity, price)',
              )
              .order('created_at', ascending: false);
          return List<Map<String, dynamic>>.from(response);
        } catch (_) {
          try {
            final response = await client
                .from('orders')
                .select(
                  'id, order_ref, user_id, status, total, created_at, updated_at, whatsapp_number, delivery_address, payment_method, courier_name, tracking_number, estimated_delivery, status_note, admin_note',
                )
                .order('created_at', ascending: false);
            return List<Map<String, dynamic>>.from(
              response,
            ...
            try {
              final response = await client
                  .from('orders')
                  .select(
                    'id, order_ref, user_id, status, total, created_at, whatsapp_number, delivery_address, payment_method',
                  )
                  .order('created_at', ascending: false);
              return List<Map<String, dynamic>>.from(
                response,
