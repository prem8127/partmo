  ...
  Future<void> _submit() async {
    ...
    final mfg = <String, String>{
      ...
      if (_hsCodeCtrl.text.trim().isNotEmpty)
        'HSN Code (N)': _hsCodeCtrl.text.trim(),
    };
    ...
    try {
      ...
      for (final sheet in workbook.tables.values) {
        ...
        for (final row in sheet.rows.skip(1)) {
          ...
          final categoryInput = value('category');
          final price = double.tryParse(value('price')) ?? 0;
          if (name.isEmpty || categoryInput.isEmpty || price <= 0) continue;

          final category = resolveProductCategory(
            categoryInput,
            _availableCategories,
          );
          final mrp = double.tryParse(value('mrp')) ?? price;
          final stock = int.tryParse(value('stock')) ?? 0;
          final description = value('description');
          final imageUrl = value('imageurl');
          final badge = value('badge');
          final hsn = value('hsncode');
          ...
          rows.add({
            ...
            'image_url': imageUrl,
            'image_urls': imageUrl.isEmpty ? <String>[] : [imageUrl],
            'compatibility': compatibility,
            'specs': <String, String>{
              if (hsn.isNotEmpty) 'HSN Code (N)': hsn,
            },
          });
        }
      }
      ...
      if (rows.isEmpty) {
        setState(() => _error =
            'No valid products found. Use columns: name, category, price, mrp, stock, description, image_url, badge, compatibility.');
        return;
      }

      await SupabaseService.insertProducts(rows);
      ref.invalidate(productsStreamProvider);
      setState(() => _success = '${rows.length} products imported.');
                        ...
                        child: SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            ...
                            icon: _importing
                                ...
                                : const Icon(Icons.upload_file_outlined),
                            label: Text(
                                _importing ? 'Importing...' : 'Import Excel'),
                          ),
                        ),
                      ),
        ...
        Container(
          ...
          child:
            ...
            Row(children: [
              ...
              const SizedBox(width: 12),
              Expanded(child: _F('HSN CODE (N)', '8708.99.10', hsCodeCtrl)),
            ]),
            const SizedBox(height: 14),
