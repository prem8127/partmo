...
class OrderTrackingScreen extends ConsumerWidget {
                      ...
                      : orders.isEmpty
                          ...
                          : ListView.separated(
                              ...
                              itemBuilder: (_, i) => _OrderCard(
                                ...
                                onStatusAction: (status, note) =>
                                    _customerStatusAction(
                                  context,
                                  ref,
                                  orders[i],
                                  status,
  ...
  Future<void> _downloadInvoice(BuildContext context, Order order) async {
    try {
      ...
      await invoice.InvoiceService.downloadInvoiceImage(invoice.InvoiceData(
        orderId: order.id,
  ...
  Future<void> _customerStatusAction(
    ...
    if (order.backendId.isEmpty) return;
    ...
    try {
      await SupabaseService.customerUpdateOrderStatus(
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Order updated to ${OrderStatus.label(status)}')),
        );
      }
    } catch (e) {
      if (context.mounted) {
...
class _OrderCard extends StatelessWidget {
  ...
  final Future<void> Function(String status, String note) onStatusAction;
  @override
  Widget build(BuildContext context) {
...
class _CustomerOrderActions extends StatelessWidget {
  const _CustomerOrderActions({required this.order, required this.onAction});

  final Order order;
  final Future<void> Function(String status, String note) onAction;
  ...
  Widget build(BuildContext context) {
    ...
    if ([OrderStatus.shipped, OrderStatus.outForDelivery].contains(status)) {
      actions.add(_ActionButton(
        label: 'I Received Order',
